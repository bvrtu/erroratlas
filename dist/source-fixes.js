...
export function renderSourceFixes(fixes) {
    ...
    return `${[
        ...
        "",
        ...fixes.map((fix) => `${fix.file}:${fix.line}:${fix.column} add code ${fix.code} for ${JSON.stringify(fix.message)}`),
    ].join("\n")}\n`;
}
function planTypeScriptFileFixes(root, filename, source) {
...
function addFix(root, filename, body, status, fixes, seen) {
