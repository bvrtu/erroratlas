...
const DEFAULT_CONSTRUCTORS: Record<SupportedLanguage, ConstructorSpec[]> = {
  typescript: [
    {
      name: "AppError",
      codeArgument: 0,
...
export async function loadConfig(root: string): Promise<ErrorAtlasConfig> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
      throw new Error(
        `Could not read ${CONFIG_FILE}: ${(error as Error).message}`,
      );
    }
  }
...
function mergeConstructors(
  defaults: ConstructorSpec[],
  custom: ConstructorSpec[],
): ConstructorSpec[] {
  const merged = new Map(defaults.map((item) => [item.name, item]));
  for (const item of custom) merged.set(item.name, item);
  ...
  for (const language of [
    ...
    for (const constructor of config.constructors?.[language] ?? []) {
      if (!/^[$A-Z_a-z][$\w]*(?:\.[$A-Z_a-z][$\w]*)*$/.test(constructor.name)) {
        throw new Error(
          `Invalid ${language} constructor name: ${constructor.name}`,
        );
      }
      for (const key of [
        "codeArgument",
        ...
        if (value !== undefined && (!Number.isInteger(value) || value < 0)) {
          throw new Error(
            `${constructor.name}.${key} must be a non-negative integer.`,
          );
        }
      }
    }
