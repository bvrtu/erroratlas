...
export type TypeScriptImportResolutionKind =
  | "relative-import"
  | "path-alias"
  | "base-url"
  | "workspace-import";
...
export async function loadTypeScriptProjectResolution(
  root: string,
  policy: TypeScriptPolicy,
): Promise<TypeScriptProjectResolution | null> {
  if (!policy.resolveProjectImports) return null;
...
export function resolveTypeScriptImport(
  ...
  project: TypeScriptProjectResolution | null,
): ResolvedTypeScriptImport | null {
  ...
  if (project.baseUrl) {
    const target = resolveCandidate(
      path.resolve(project.baseUrl, specifier),
      knownFiles,
      project.root,
    );
    if (target) return { filename: target, kind: "base-url" };
  }
...
export function typeScriptImportSpecifiers(source: string): string[] {
...
async function loadTsConfig(
  ...
  if (depth > MAX_TSCONFIG_EXTENDS) {
    throw new Error(
      `TypeScript config extends more than ${MAX_TSCONFIG_EXTENDS} local files.`,
    );
  }
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      throw new Error(
        `TypeScript project resolution is enabled but ${path.relative(root, filename)} was not found.`,
      );
    }
    throw new Error(
      `Could not read TypeScript config ${path.relative(root, filename)}: ${(error as Error).message}`,
    );
  }

  let inherited: LoadedTsConfig = { baseUrl: null, aliases: [] };
  if (typeof raw.extends === "string") {
    if (!raw.extends.startsWith(".")) {
      throw new Error(
        `TypeScript project resolution supports only local tsconfig extends; received ${raw.extends}.`,
      );
    }
    const parent = resolveConfigFilename(
      path.resolve(path.dirname(filename), raw.extends),
    );
  ...
  const baseUrl =
    typeof compilerOptions.baseUrl === "string"
      ...
      : inherited.baseUrl;
  if (baseUrl) assertWithin(root, baseUrl, "TypeScript baseUrl");
...
function parseAliasRules(
  ...
  for (const [pattern, rawTargets] of Object.entries(paths)) {
    ...
    if (count(pattern, "*") > 1) {
      throw new Error(
        `TypeScript path pattern has more than one *: ${pattern}`,
      );
    }
    ...
    const targets = rawTargets
      ...
      .map((target) => {
        if (count(target, "*") > 1) {
          throw new Error(
            `TypeScript path target has more than one *: ${target}`,
          );
        }
    ...
    if (targets.length) rules.push({ pattern, prefix, suffix, targets });
  }
  return rules.sort(
    (left, right) =>
...
async function loadWorkspacePackages(
  ...
  const patterns = Array.isArray(manifest.workspaces)
    ...
    : isRecord(manifest.workspaces) &&
      ...
      : [];
  const safePatterns = patterns.filter((item): item is string => {
    ...
    if (
      ...
      throw new Error(
        `Workspace pattern must stay inside the project: ${item}`,
      );
    }
    return true;
  });
  if (!safePatterns.length) return [];
  ...
  for (const relative of manifests.sort()) {
    ...
    const child = JSON.parse(await readFile(filename, "utf8")) as Record<
      string,
      unknown
    >;
...
function resolveAlias(
  specifier: string,
  knownFiles: ReadonlySet<string>,
  project: TypeScriptProjectResolution,
): { matched: boolean; filename: string | null } {
  const matches = project.aliases
    .map((rule) => ({ rule, capture: matchPattern(rule, specifier) }))
    .filter(
      (item): item is { rule: AliasRule; capture: string } =>
        item.capture !== null,
    );
  if (!matches.length) return { matched: false, filename: null };
  const best = matches[0];
  if (!best) return { matched: false, filename: null };
  const bestScore = aliasScore(best.rule);
  const resolved = new Set<string>();
  for (const { rule, capture } of matches) {
    if (aliasScore(rule) !== bestScore) break;
...
function resolveWorkspace(
  ...
  for (const candidate of candidates) {
    ...
    if (!isWithin(workspace.root, absolute)) continue;
...
function exportedTarget(value: unknown, subpath: string): string | null {
  if (typeof value === "string") return subpath === "." ? value : null;
  if (!isRecord(value)) return null;
  const direct = value[subpath];
  if (direct !== undefined) return conditionalTarget(direct);
  for (const [pattern, target] of Object.entries(value)) {
    if (count(pattern, "*") !== 1) continue;
    const [prefix = "", suffix = ""] = pattern.split("*");
...
function conditionalTarget(value: unknown): string | null {
...
function matchPattern(rule: AliasRule, specifier: string): string | null {
...
function aliasScore(rule: AliasRule): number {
  return (
    (rule.pattern.includes("*") ? 0 : 1_000_000) +
    rule.prefix.length +
    rule.suffix.length
  );
}
...
function resolveCandidate(
  ...
  return (
    candidates.find(
      (candidate) =>
        (!root || isWithin(root, candidate)) && knownFiles.has(candidate),
    ) ?? null
  );
}

function resolveConfigFilename(value: string): string {
...
function stripTrailingCommas(source: string): string {
  ...
  for (let index = 0; index < source.length; index += 1) {
    ...
    if (character === ",") {
      ...
      if (["}", "]"].includes(source[cursor] ?? "")) continue;
    }
    output += character;
  }
  return output;
